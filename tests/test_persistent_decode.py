...
def model(request):
    ...
    return OuroForCausalLM(OuroConfig.tiny()).to(dtype=request.param)
...
def test_fixed_storage_payload_and_snapshot_are_detached(model):
    ...
    assert snapshot["device_payload_bytes"] == (
        (16 * model.config.hidden_size + 8) * parameter.element_size() + 1288
    )
    for name in ("hidden_in", "hidden_out", "gate_out"):
        assert snapshot["tensors"][name]["dtype"] == str(parameter.dtype)
...
def test_constructor_cap_dtype_and_partial_failure_leave_executor_disabled(model, monkeypatch):
    ...
    other_dtype = torch.bfloat16 if cache.dtype == torch.float32 else torch.float32
    model.to(dtype=other_dtype)
    with pytest.raises(ValueError, match="same dtype"):
        runner._enable_persistent_decode()
    model.half()
    with pytest.raises(ValueError, match="bfloat16 and float32"):
        runner._enable_persistent_decode()
...
def test_persistent_matches_allocating_padded_and_published_state_survives_reuse(
    ...
    torch.testing.assert_close(torch.tensor(actual), expected[1].float().sigmoid(), atol=0, rtol=0)
...
def test_direct_calls_complete_before_lease_release_and_empty_is_noop(model, monkeypatch):
    ...
    runner._recurrent_persistent(
        next(model.parameters()).new_empty(0, model.config.hidden_size), [], [], []
    )
...
def test_fallback_counts_real_compact_dispatch_without_mutating_buffers(model, monkeypatch, reason):
    ...
    def compact(hidden, request_ids, depths, positions, owner):
        ...
        return hidden + 1, hidden.new_zeros(count)

    monkeypatch.setattr(model, "recurrent", compact)
    runner._recurrent_persistent(
        next(model.parameters()).new_zeros(count, model.config.hidden_size),
        ids,
        [0] * count,
        [position] * count,
    )
    ...
    with pytest.raises(ValueError, match="duplicate"):
        runner._recurrent_persistent(
            next(model.parameters()).new_zeros(5, model.config.hidden_size),
            [ids[0]] * 5,
            [0] * 5,
            [position] * 5,
        )
...
def test_persistent_prefix_guard_still_requires_each_layer_write(model):
    ...
    x = torch.full((8, cache.num_kv_heads, cache.head_dim), torch.nan, dtype=cache.dtype)
    x[1] = 3
